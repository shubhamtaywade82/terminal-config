...
**Linux (Debian / Ubuntu / WSL2, `apt`):** use `./install-linux.sh` from the repo root (`--dry-run` first if you like). It installs overlapping tools via `apt`, uses upstream installers when a package is missing from your release, writes `~/.fzf.zsh` for Debian-style fzf paths, symlinks `bat`/`fd` when the distro ships `batcat`/`fdfind`, and runs the same optional global Claude Code install when `npm` exists. Flags: `--skip-optional`, `--skip-claude`, `--skip-fzf-setup`, `--dotfiles-status`. It does **not** edit `~/.zshrc` or Starship/Cursor JSON—merge as for macOS. Fedora/Arch or Linuxbrew: install the same tools with your package manager; see [FEATURES_AND_USAGE.md](./FEATURES_AND_USAGE.md).
